...
function getSearchValue() {
  return (searchInput?.value ?? filterInput?.value ?? "").trim();
}
    ...
    if (resultsContainer) {
      resultsContainer.innerHTML = `
        <p class="empty-message">
          Não foi possível carregar os dados. Verifique se o arquivo data.json está na raiz do projeto.
        </p>
      `;
    }
...
function render(list = data) {
  ...
  resultsContainer.innerHTML = list
    .map((item) => {
      const nome = escapeHTML(item.nome || "Nome não informado");
      const categoria = escapeHTML(item.categoria || "Serviço");
      const descricao = escapeHTML(item.descricao || "Descrição não informada.");
      const contato = item.contato ? escapeHTML(item.contato) : "";
      const endereco = item.endereco ? escapeHTML(item.endereco) : "";
      const link = item.link ? String(item.link).trim() : "";

      return `
        <article class="card reveal">
          <span class="tag-soft">${categoria}</span>
          <h3>${nome}</h3>
          <p>${descricao}</p>
            ...
            link
              ? `
                <a
                  ...
                  aria-label="Ver mais detalhes sobre ${nome}"
                >
                  Ver mais detalhes
                </a>
              `
  ...
  if (resultCount) {
    resultCount.textContent = `${list.length} ${
      list.length === 1 ? "resultado encontrado" : "resultados encontrados"
    }`;
  }
...
function carregarPreferencias() {
  try {
    ...
    localStorage.removeItem("teaPreferencias");
  }
}
...
function updateCounter() {
  document.querySelectorAll("[data-target]").forEach((el) => {
    const target =
      el.dataset.target === "0" ? data.length : Number(el.dataset.target) || 0;
...
function updateMapMarkers(list = data) {
  ...
  placesWithLocation.forEach((item) => {
    const nome = escapeHTML(item.nome || "Local não informado");
    const categoria = escapeHTML(item.categoria || "");
    const endereco = escapeHTML(item.endereco || "");

    L.marker([item.lat, item.lng]).addTo(markersLayer).bindPopup(`
